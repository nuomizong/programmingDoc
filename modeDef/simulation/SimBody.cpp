...
CBody::CBody(const CBodyParameters& p) 
{
  ...
  std::sprintf(statefilename, "models/%s.prop", bodyname);
  fprop.open(statefilename);
  if(!fprop) {
	  std::cout << "Can't open property file " << statefilename << std::endl;
    exit(1);
  }


  fprop >> temp;
  std::sprintf(datafilename, "models/%s", temp);
  ...
  //if ( Deformable ) {
    ...
    if(!data) {
		std::cout << "Can't open deformable object file " << datafilename << std::endl;
      exit(1);
    }
...
void CBody::GetClosestBodypoint(const Vector3& pt, CBodyPoint& p) const
{
  ...
  tetrahedron_ptr *t = geometry->object.tets.elements;
  ...
  if(min_dist <= 1.001f) {  //it's inside the tet
	...
	std::cout << "constraint point " << pt << std::endl;
	std::cout << "surface point " << x << std::endl;
