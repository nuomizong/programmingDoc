...
#include "simulation/SimHelpers.h"
#include "simulation/SimAnim.h"
#include "math/svd.h"
#include "math/bary.h"
#include "shared/myarray.h"
...
void CSimulation::SpringForces()
{
  ...
  while((p = constraints.next(p))) {
    if(p->IsPenetration()) {
      ...
      if(verbose >= 2)
		  std::cout << "adding spring force " << f << std::endl;
...
void    CSimulation::readSetupFile(std::ifstream& fsetup)
{
    fsetup >> Use_Penalty;
...
std::ostream& operator << (std::ostream& out, const CRigidState& s)
{
...
std::istream& operator >> (std::istream& in, CRigidState& s)
{
    Vector3     P, V, AV;
    Matrix3   R;
    QuaternionRotation Q;
...
std::ostream& operator << (std::ostream& out, const CBodyPoint& p)
{
	out << p.n << std::endl;
  for(int i=0; i<p.n; i++) {
	  out << p.vert[i] << " " << p.coords[i] << std::endl;
  }
  return out;
}

std::istream& operator >> (std::istream& in, CBodyPoint& p)
{
  in >> p.n;
  for(int i=0; i<p.n; i++) {
...
std::ostream& operator << (std::ostream& out, const CBodyParameters& p)
{
  out << p.name;
  ...
  out << p.birthTime << " " << p.deathTime << std::endl;
  if(!p.animationPath.empty()) {
	  out << p.animationPath << std::endl;
  }
  return out;
}

std::istream& operator >> (std::istream& in, CBodyParameters& p)
{
  char str [256];
  str[0] = 0;
  while(str[0] == 0) {
  ...
  if(nameend != -1) {
	...
	break;
      default:
		  std::cout << "Error reading options: " << p.name.substr(i, n - i).c_str() << std::endl;
		  //in.set(std::ios::badbit);
	return in;
	break;
      }
    }
