...
std::istream &operator>>(std::istream &buf, BoundBox &bb) {
	long orgFlags = buf.setf(std::ios::skipws);
  eatChar('{',buf);
  buf >> bb.bmin;
  eatChar(',',buf);
  buf >> bb.bmax;
...
std::ostream &operator<<(std::ostream &buf, const BoundBox &bb) {
  buf << '{';
  buf << bb.bmin;
  buf << ',';
  buf << bb.bmax;
...
void BoundTree::setInnerLevelSize(unsigned short level    , 
  ...
  if (level >= _numInnerLevels) {
	  std::cerr << "BoundTree::setInnerLevelSize range error.\n" << std::flush;
    abort();
  }
...
std::istream &operator>>(std::istream &buf, BoundTree::innerNode &n) {
	long orgFlags = buf.setf(std::ios::skipws);
  eatChar('{',buf);
  buf >> n.bound;
  eatChar(',',buf);
...
std::ostream &operator<<(std::ostream &buf, const BoundTree::innerNode &n) {
  buf << '{';
  buf << n.bound;
  buf << ',';
  buf << n.childLevel;
...
std::istream &operator>>(std::istream &buf, BoundTree::leafNode &n) {
	long orgFlags = buf.setf(std::ios::skipws);
  eatChar('{',buf);
  buf >> n.bound;
  eatChar(',',buf);
...
std::ostream &operator<<(std::ostream &buf, const BoundTree::leafNode &n) {
  buf << '{';
  buf << n.bound;
  buf << ',';
  buf << n.mapIndex;
...
std::istream &operator>>(std::istream &buf, BoundTree &t) {
	long orgFlags = buf.setf(std::ios::skipws);
...
std::ostream &operator<<(std::ostream &buf, const BoundTree &t) {
...
void BoundOverlapServer::blowChunks(){
  if (_pendListHead != NULL) {
	  std::cerr << "BoundOverlapServer::blowChunks() :: Error: Pending list is not empty while blowing chunks.\n";
	  std::cerr << "BoundOverlapServer::blowChunks() :: Atempting to continue (get ready for a core dump...)  \n";
  }
...
void BoundOverlapServer::doneWithCache() {
  if ((_cacheUse == CNotReady) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::doneWithCache :: Error: Cache was not ready...\n";
    abort();
  }
...
int BoundOverlapServer::traverseCache(LeafOverlapCB cb, 
  ...
  if ((_cacheUse != CReady) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::traverseCache :: Error: Cache was not ready...\n";
    abort();
  }
  ...
  while ((pos != NULL) && ((*pos) != NULL)) {
	...
	(treeTwo.numInnerLevels() != l2)) {
		std::cerr << "BoundOverlapServer::traverseCache :: Error bad trees.\n";
      abort();
    }
...
int BoundOverlapServer::traverseSelfCache(SelfOverlapCB cb, 
  ...
  if ((_cacheUse != CReady) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::traverseSelfCache :: Error: Cache was not ready...\n";
    abort();
  }
  ...
  while ((pos != NULL) && ((*pos) != NULL)) {
	...
	(tree.numInnerLevels() != l2)) {
		std::cerr << "BoundOverlapServer::traverseSelfCache :: Error bad trees.\n";
      abort();
    }
...
int BoundOverlapServer::traverseRayCache(RayOverlapCB cb, 
  ...
  if ((_cacheUse != CReadyRay) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::traverseRayCache :: Error: Cache was not ready...\n";
    abort();
  }
  overlap ** pos = &_cacheListHead;
  ...
  while ((pos != NULL) && ((*pos) != NULL)) {
    ...
    if (tree.numInnerLevels() != l1) {
		std::cerr << "BoundOverlapServer::traverseRayCache :: Error bad trees.\n";
      abort();
    }
...
int BoundOverlapServer::traversePlaneCache(PlaneOverlapCB cb, 
  ...
  if ((_cacheUse != CReadyPlane) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::traversePlaneCache :: Error: Cache was not ready...\n";
    abort();
  }
  overlap ** pos = &_cacheListHead;
  int result = 0;
  ...
  while ((pos != NULL) && ((*pos) != NULL)) {
    ...
    if (tree.numInnerLevels() != l1) {
		std::cerr << "BoundOverlapServer::traversePlaneCache :: Error rotten trees.\n";
      abort();
    }
...
int BoundOverlapServer::traverseSphereCache(SphereOverlapCB cb, 
  ...
  if ((_cacheUse != CReadySphere) || (!_isInUse)) {
	  std::cerr << "BoundOverlapServer::traverseSphereCache :: Error: Cache was not ready...\n";
    abort();
  }
  overlap ** pos = &_cacheListHead;
  int result = 0;
  ...
  while ((pos != NULL) && ((*pos) != NULL)) {
    ...
    if (tree.numInnerLevels() != l1) {
		std::cerr << "BoundOverlapServer::traverseSphereCache :: Error rotten trees.\n";
      abort();
    }
...
int BoundOverlapServer::doTraversal(LeafOverlapCB cb, 
  ...
  if (!isReady()) {
	  std::cerr << "BoundOverlapServer::doTraversal :: Error: Traversal attempted on not ready server.\n";
    abort();
  }
...
int BoundOverlapServer::doSelfTraversal(SelfOverlapCB cb, 
  ...
  if (!isReady()) {
	  std::cerr << "BoundOverlapServer::doSelfTraversal :: Error: Traversal attempted on not ready server.\n";
    abort();
  }
...
int BoundOverlapServer::doSphereTraversal(SphereOverlapCB cb, 
  ...
  if (!isReady()) {
	  std::cerr << "BoundOverlapServer::doSphereTraversal :: Error: Traversal attempted on not ready server.\n";
    abort();
  }
...
int BoundOverlapServer::doPlaneTraversal(PlaneOverlapCB cb, 
  ...
  if (!isReady()) {
	  std::cerr << "BoundOverlapServer::doPlaneTraversal :: Error: Traversal attempted on not ready server.\n";
    abort();
  }
