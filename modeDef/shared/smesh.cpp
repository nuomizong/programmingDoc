...
std::istream& load_vec3(std::istream& in, vec3_t v)
{
  in >> v[0] >> v[1] >> v[2];
  return in;
}

std::istream& operator >> (std::istream& in, surface_mesh::texcoord_t t)
{
  in >> t[0] >> t[1];
  return in;
}

std::istream& load_rgba(std::istream& in, surface_mesh::rgba_t c)
{
  in >> c[0] >> c[1] >> c[2] >> c[3];
  return in;
}

std::istream& operator >> (std::istream& in, surface_mesh::vertex_ref_t& v)
{
  in >> v.vertex >> v.normal >> v.texcoords;
  return in;
}

std::istream& load_face(std::istream& in, surface_mesh::face_t f)
{
  in >> f[0] >> f[1] >> f[2];
  return in;
}

std::istream& operator >> (std::istream& in, surface_mesh::material_t& m)
{
...
std::istream& operator >> (std::istream& in, surface_mesh::face_group_t& f) {
  in >> f.face_start >> f.face_end >> f.material;
  return in;
};

bool surface_mesh::load(std::istream& f) {
...
std::ostream& save_vec3(std::ostream& out, const vec3_t v)
{
  out << v[0] << " "
	  << v[1] << " "
	  << v[2];
...
std::ostream& operator << (std::ostream& out, const surface_mesh::texcoord_t t)
{
  out << t[0] << " " 
	  << t[1];
  return out;
...
std::ostream& save_rgba(std::ostream& out, const surface_mesh::rgba_t c)
{
  out << c[0] << " "
	  << c[1] << " "
	  << c[2] << " "
...
std::ostream& operator << (std::ostream& out, const surface_mesh::vertex_ref_t& v)
{
  out << v.vertex << " " << v.normal << " " << v.texcoords;
  return out;
}

std::ostream& save_face(std::ostream& out, const surface_mesh::face_t f)
{
  out << f[0] << " " << f[1] << " " << f[2];
  return out;
}

std::ostream& operator << (std::ostream& out, const surface_mesh::material_t& m)
{
	save_rgba(out, m.ambient); out << std::endl;
	save_rgba(out, m.diffuse); out << std::endl;
	save_rgba(out, m.specular); out << std::endl;
	save_rgba(out, m.emissive); out << std::endl;
  out << m.specular_strength;
  return out;
}

std::ostream& operator << (std::ostream& out, const surface_mesh::face_group_t& f) {
  out << f.face_start << " " << f.face_end << "\t" << f.material;
  return out;
};

bool surface_mesh::save(std::ostream& f) {
  f << num_vertices << " " << num_normals << " " << num_texcoords << " "
	  << num_faces << " " << num_materials << " " << num_face_groups << std::endl;
  ...
  int i;
  for(i=0; i<num_vertices; i++) {
	  save_vec3(f, vertices[i]); f << std::endl;
  }
  f << std::endl;

  for(i=0; i<num_normals; i++) {
	  save_vec3(f, normals[i]); f << std::endl;
  }
  f << std::endl;

  for(i=0; i<num_texcoords; i++)
	  f << texcoords[i] << std::endl;
  f << std::endl;

  for(i=0; i<num_faces; i++) {
	  save_face(f, faces[i]); f << std::endl;
  }
  f << std::endl;
